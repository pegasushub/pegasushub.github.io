...
def validate_yaml(data):
    ...
    for error in errors:
        if error.validator == "anyOf":
            ...
            print(error.message, " in ", error.schema_path[-2], " at index ", error.json_path.split("]")[0][2:])
            logger.error(f"{error.message} in {error.schema_path[-2]} at index {error.json_path.split(']')[0][2:]}")
        else:
            print(error.message)
